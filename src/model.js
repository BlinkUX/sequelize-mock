...
function fakeModel(name, defaults, opts) {
	...
	this.options = _.extend(
		...
		_.pick(
			...
			["autoQueryFallback", "stopPropagation", "fallbackFn"]
		),
		opts || {}
	);
	...
	this.tableName =
		...
		(this.options.freezeTableName ? name : Utils.pluralize(name));
	 ...
	 **/
	this.$queueResult = this.$queryInterface.$queueResult.bind(
		this.$queryInterface
	);
	/**
	 ...
	 * @return {QueryInterface} model instance of QueryInterface
	 **/
	this.$queueFailure = this.$queueError = this.$queryInterface.$queueFailure.bind(
		this.$queryInterface
	);
	/**
	 ...
	 * @return {QueryInterface} model instance of QueryInterface
	 **/
	this.$clearQueue = this.$queueClear = this.$queryInterface.$clearQueue.bind(
		this.$queryInterface
	);
 ...
 **/
fakeModel.prototype.findAndCount = fakeModel.prototype.findAndCountAll = function(
	options
) {
	var self = this;
 ...
 **/
fakeModel.prototype.find = fakeModel.prototype.findOne = function(obj) {
	var self = this;
 ...
 **/
fakeModel.prototype.insertOrUpdate = fakeModel.prototype.upsert = function(
	values
) {
	var self = this;
...
fakeModel.prototype.destroy = function(options) {
	...
	return this.$query({
		...
		fallbackFn: !this.options.autoQueryFallback
			...
			: function() {
					return Promise.resolve(
						options && typeof options.limit == "number" ? options.limit : 1
					);
			  }
	});
};
...
// Associations
fakeModel.prototype.belongsTo = fakeModel.prototype.hasOne = function(
	item,
	options
) {
	...
	if (isString) {
		this.Instance.prototype["get" + singular] = function(opts) {
			return Promise.resolve(
				new self.Instance(opts && opts.where ? opts.where : opts)
			);
		};
	} else {
	...
	this.Instance.prototype["create" + singular] = item.create
		? item.create.bind(item)
		: noop;
};

fakeModel.prototype.belongsToMany = fakeModel.prototype.hasMany = function(
	item,
	options
) {
	if (!(item instanceof fakeModel)) {
		return {
			through: {
				model: new fakeModel(
					this.getTableName() + (item && item.name ? item.name : "Association"),
					{},
	...
	if (isString) {
		this.Instance.prototype["get" + plural] = function(opts) {
			return Promise.resolve([
				new self.Instance(opts && opts.where ? opts.where : opts)
			]);
		};
	...
	this.Instance.prototype["count" + plural] = function() {
