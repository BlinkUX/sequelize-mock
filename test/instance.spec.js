...
describe('Instance', function () {
	...
	describe('#get', function () {
		...
		it('should get all the values of the object', function () {
			var inst = new Instance();
			inst._values.foo = 'bar';
			inst._values.run = 'far';

			const vals = inst.get();
			vals.foo.should.be.exactly('bar');
			vals.run.should.be.exactly('far');
		});
	});
	...
	describe('#set', function () {
		it('should set the value of a property on the object', function () {
			...
			inst._values.foo = 'bar';
            inst.set('foo', 'baz');
			inst.get('foo').should.be.exactly('baz');
		});

		it('should set multiple properties if an object is given', function () {
			var inst = new Instance();
			inst._values.foo = 'bar';
			inst._values.run = 'far';
            inst.set({ foo: 'baz', run: 'farther'});

			const vals = inst.get();
			vals.foo.should.be.exactly('baz');
			vals.run.should.be.exactly('farther');
		});
	});
